...
1) **Apply DB migration** and verify RLS with real JWT claims; seed minimal data if needed. Helper: `supabase/tests/nft_trading_rls_checks.sql` (run via psql) to assert anon/auth/owner behaviors.
2) **Indexer validation** on target RPC + Supabase; confirm cursor replay and event upserts.
